...
export default function StockReport({
    ...
    const totalStockIn = reportsList.reduce(
        (sum, r) => sum + (parseFloat(r.stock_in) || 0),
        0,
    );
    const totalStockOut = reportsList.reduce(
        (sum, r) => sum + (parseFloat(r.stock_out) || 0),
        0,
    );
    ...
    return (
        <AppShell title="Stock Report" breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                        Comprehensive audit report of material stock levels,
                        stock movement transactions, and closing SOH balances.
                    </p>
                </div>
                <a
                    href={getExportUrl()}
            ...
            <form
                ...
                className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs mb-6 space-y-3"
            >
                ...
                <div className="px-4 py-3 bg-slate-50/80 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-center space-x-1 bg-slate-200/70 p-1 rounded-lg">
                        ...
                        >
                            ...
                            <span>
                                Stock Movements (
                                {meta.total ?? movementData.length})
                            </span>
                        </button>
                        <button
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-4 py-3 text-slate-600">
                                                        {tx.material_request?.no_doc || tx.doc_archive_ref ||
                                                            "-"}
                                                    </td>
                                                    <td className="px-4 py-3 text-right text-emerald-700 font-mono font-bold">
                                                        {parseFloat(
                                                            ...
                                                            ? `+${parseFloat(tx.qty_in).toFixed(2)}`
                                                            : "-"}
                                                    </td>
                                                    <td className="px-4 py-3 text-right text-red-700 font-mono font-bold">
                                                        {parseFloat(
                                                            ...
                                                            ? `-${parseFloat(tx.qty_out).toFixed(2)}`
                                                            : "-"}
                                                    </td>
