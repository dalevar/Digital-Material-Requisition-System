...
import { FileText, CheckCircle2, XCircle, Clock, Download, Filter, RotateCcw } from 'lucide-react';
  ...
  return (
    <AppShell title="Approval Report">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-900">Approval Audit & History Report</h1>
          <p className="text-xs text-slate-500 mt-1">
            Comprehensive audit report tracking material requisition approvals, rejections, and approver decisions.
          </p>
        </div>
        <a
          href={getExportUrl()}
        ...
        <form onSubmit={handleFilterSubmit} className="flex flex-wrap items-end gap-3 text-xs">
          <div className="flex-1 min-w-[160px]">
            ...
            >
              ...
              <option value="CANCELLED_AFTER_APPROVAL">CANCELLED_AFTER_APPROVAL</option>
            </select>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            {(filterForm.status || filterForm.date_from || filterForm.date_to) && (
              <button
                type="button"
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
          ...
          <table className="w-full text-left text-xs text-slate-600">
            <thead className="bg-slate-50 text-slate-700 font-bold uppercase text-[10px] tracking-wider border-b border-slate-100">
              <tr>
                ...
                <th className="p-4">Action / Rejection Reason</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {requestList.length > 0 ? (
                requestList.map((req) => {
                  const latestHistory = req.approvalHistories && req.approvalHistories.length > 0
                    ? req.approvalHistories[req.approvalHistories.length - 1]
                    : null;
                  const reason = req.rejection_reason || latestHistory?.reason || '-';

                  return (
                    <tr key={req.id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="p-4 font-bold text-slate-900">{req.request_no}</td>
                      <td className="p-4 text-slate-500 font-medium">{req.no_doc || '-'}</td>
                      <td className="p-4 font-semibold text-slate-800">{req.requester?.name || '-'}</td>
                      <td className="p-4 text-slate-600">{req.department?.name || '-'}</td>
                      <td className="p-4 text-slate-600">{req.request_date || '-'}</td>
                      <td className="p-4 font-medium text-slate-800">{req.approver?.name || '-'}</td>
                      <td className="p-4 text-center">
                        <StatusBadge status={req.status} />
                      </td>
                      <td className="p-4 text-slate-600 max-w-xs truncate" title={reason}>
                        {reason}
                      </td>
                    </tr>
                  );
        ...
        {requests?.links && requests.links.length > 3 && (
          <div className="p-4 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
            <div>
              Showing {requests.from || 0} to {requests.to || 0} of {requests.total || 0} records
            </div>
