...
export default function Departments({ departments = { data: [] }, filters = {} }) {
  const breadcrumbs = [
  ...
  const resetFilters = () => {
    setSearch('');
    setStatusFilter('');
    router.get('/admin/departments', {}, { preserveScroll: true });
  };
  ...
  const meta = departments?.meta ?? departments;

  return (
    <AppShell title="Departments Master Data" breadcrumbs={breadcrumbs}>
      <Head title="Departments - DMRS" />

      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">Departments Master Data</h1>
          <p className="text-xs text-slate-500">Manage organizational departments and operational units.</p>
        </div>
        <button
              ...
              <tbody className="divide-y divide-slate-200">
                {items.map((dept) => (
                  <tr key={dept.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 font-mono font-bold text-red-700">{dept.code}</td>
