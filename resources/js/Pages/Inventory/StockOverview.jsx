...
export default function StockOverview({ materials = { data: [] } }) {
    ...
    const handleMaterialChangeForStockIn = (matId) => {
        const mat =
            materialList.find((m) => String(m.id) === String(matId)) || null;
        setSelectedMat(mat);
        stockInForm.setData((prev) => ({
            ...prev,
            material_id: matId,
            storage_location: mat
    ...
    const currentSohIn = selectedMat
        ? parseFloat(selectedMat.stock_balance?.quantity || 0)
        : 0;
    const qtyInInput = parseFloat(
        stockInForm.data.quantity || stockInForm.data.qty || 0,
    );
    const finalStockInPreview =
        currentSohIn + (isNaN(qtyInInput) ? 0 : qtyInInput);

    // Stock Adjustment Preview Calculation & Validation
    const currentSohAdj = selectedMat
        ? parseFloat(selectedMat.stock_balance?.quantity || 0)
        : 0;
    const adjQtyInput = parseFloat(adjForm.data.adjustment_quantity || 0);
    const finalStockAdjPreview =
        currentSohAdj + (isNaN(adjQtyInput) ? 0 : adjQtyInput);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    <h1 className="text-xl font-bold text-slate-900 tracking-tight">
                        Inventory Stock Overview & SOH Control
                    </h1>
                    <p className="text-xs text-slate-500">
                        Real-time Stock On Hand (SOH), Min/Max thresholds, and
                        ...
                        <table className="w-full text-left text-xs text-slate-700">
                            ...
                            <tbody className="divide-y divide-slate-200">
                                {materialList.map((mat) => {
                                    const soh = parseFloat(
                                        mat.stock_balance?.quantity || 0,
                                    );
                                    const minStock = parseFloat(
                                        mat.minimum_stock || 0,
                                    );
                                    const maxStock = parseFloat(
                                        mat.maximum_stock || 0,
                                    );
                    ...
                    <div className="bg-white max-w-lg w-full rounded-lg p-6 shadow-xl space-y-4 my-8">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                            <div>
                                ...
                                <p className="text-xs text-slate-500">
                                    Record incoming stock batch to inventory
                                    catalog.
                                </p>
                            </div>
                        ...
                        >
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) =>
                                        handleMaterialChangeForStockIn(
                                            e.target.value,
                                        )
                                    }
                                ...
                                <div className="p-3 bg-slate-50 border border-slate-200 rounded-md text-xs space-y-1.5">
                                    <div className="flex justify-between items-center">
                                        <span className="font-bold text-red-700">
                                            {selectedMat.material_number}
                                        </span>
                                        <StockStatusBadge
                                            soh={currentSohIn}
                                            minStock={parseFloat(
                                                selectedMat.minimum_stock || 0,
                                            )}
                                        />
                                    </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input
                                        type="date"
                                        value={
                                            stockInForm.data.transaction_date
                                        }
                                        onChange={(e) =>
                                            stockInForm.setData(
                                                "transaction_date",
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto">
                    <div className="bg-white max-w-lg w-full rounded-lg p-6 shadow-xl space-y-4 my-8">
                        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                            <div>
                                <h3 className="text-base font-bold text-slate-900">
                                    Stock Physical Adjustment
                                </h3>
                                <p className="text-xs text-slate-500">
                                    Correct inventory balance based on physical
                                    stock take.
                                </p>
                            </div>
                        ...
                        <form onSubmit={handleAdjSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-500 mt-0.5">
                                        Use positive for increase (+20),
                                        negative for decrease (-15).
                                    </p>
                            ...
                            {selectedMat && (
                                ...
                                >
                                    <div className="font-bold text-[11px] uppercase tracking-wider">
                                        Calculated Final Stock Preview
                                    </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    onChange={(e) =>
                                        adjForm.setData("note", e.target.value)
                                    }
