...
export default function Edit({
  ...
  const initialItems = (request.items || []).map((item) => {
    ...
    const soh = selectedMat ? parseFloat(selectedMat.soh || 0) : parseFloat(item.soh || 0);
    const qty = parseFloat(item.qty || 0);
    return {
      id: item.id,
      material_id: item.material_id,
      description: item.description || (selectedMat ? selectedMat.description : ''),
  ...
  return (
    <AppShell title={`Edit Request ${request.request_no}`} breadcrumbs={breadcrumbs}>
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          >
            <ArrowLeft className="w-3.5 h-3.5 mr-1" /> Back to Request Details
          </Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-xs space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {isAdmin && (
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setData('requester_id', e.target.value)}
                ...
                >
                  ...
                  {requesters.map((req) => (
                    <option key={req.id} value={req.id}>
                      {req.name} ({req.employee_id || 'ID: ' + req.id})
                    </option>
                  ))}
                </select>
                {errors.requester_id && (
                  <p className="text-xs text-red-600 mt-1 font-medium">{errors.requester_id}</p>
                )}
              </div>
            )}
            ...
            <div>
              ...
              />
              {errors.request_date && (
                <p className="text-xs text-red-600 mt-1 font-medium">{errors.request_date}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errors.approver_id && (
                <p className="text-xs text-red-600 mt-1 font-medium">{errors.approver_id}</p>
              )}
            </div>
              ...
              <thead className="bg-slate-100 text-[10px] font-bold uppercase tracking-wider text-slate-600 border-b border-slate-200">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-right w-32">
                    Req Qty <span className="text-red-600">*</span>
                  </th>
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="flex items-center space-x-2 text-xs text-slate-500">
            ...
            <span>
              Estimated Balance is automatically computed: <code>Balance = SOH - Requested Qty</code>.
            </span>
          </div>
