...
export default function Show({ request, plants = [] }) {
    ...
    const handleConfirmStockOut = () => {
        ...
        router.post(
            `/admin/requests/${request.id}/issue-stock`,
            {},
            {
                onFinish: () => {
    ...
    const getCurrentStepIndex = () => {
        if (
            ...
            request.status === "CANCELLED_AFTER_APPROVAL" ||
    ...
    return (
        ...
        >
            ...
            <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                        ...
                        <span>Download Official PDF MR</span>
                    </a>
                ...
                request.status === "CANCELLED" ? (
                    <div className="p-4 rounded-md bg-red-50 border border-red-200 flex items-center space-x-3 text-red-900 text-xs">
                        ...
                        <div>
                            ...
                            This requisition has reached end-state status:{" "}
                            <StatusBadge status={request.status} />
                        </div>
                    </div>
                ) : (
                            ...
                            return (
                                <React.Fragment key={step.key}>
                                    <div className="flex flex-col items-center min-w-[90px]">
                                        ...
                                        <span
                                            className={`text-[11px] font-semibold mt-1.5 text-center ${isCompleted || isCurrent ? "text-slate-900" : "text-slate-400"}`}
                                        >
                                            {step.label}
                                        </span>
                                    </div>
                                    {idx < steps.length - 1 && (
                                        <div
                                            className={`flex-1 h-0.5 mx-2 min-w-[20px] ${idx < currentStepIdx ? "bg-emerald-600" : "bg-slate-200"}`}
                                        />
                                    )}
                                </React.Fragment>
                            );
            ...
            <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-xs mb-6 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                    <div>
                        ...
                        <div className="text-[11px] text-slate-400">
                            ID: {request.requester?.employee_id || "-"}
                        </div>
                    </div>
                    <div>
                        <div className="text-slate-500 font-medium">
                ...
                {request.reason && (
                    <div className="pt-2 border-t border-slate-100">
                        ...
                        <p className="text-xs text-slate-800 font-normal mt-0.5 leading-relaxed">
                            {request.reason}
                        </p>
                    </div>
                )}
                            ...
                            {request.items &&
                                request.items.map((item, idx) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-2.5 font-bold text-red-700">
                                        ...
                                        <td className="px-3 py-2.5 text-right font-mono text-slate-700">
                                            {parseFloat(item.soh || 0).toFixed(
                                                2,
                                            )}
                                        </td>
                                        <td
                                            className={`px-3 py-2.5 text-right font-mono font-bold ${item.balance < 0 ? "text-red-600" : "text-emerald-600"}`}
                                        >
                                            {parseFloat(
                                                item.balance || 0,
                                            ).toFixed(2)}
            ...
            <div className="bg-white rounded-lg border border-slate-200 shadow-xs p-6 mb-6">
                ...
                {request.approval_histories &&
                request.approval_histories.length > 0 ? (
                    <div className="space-y-3">
                        {request.approval_histories.map((hist) => (
                            <div
                                key={hist.id}
                                className="flex items-start space-x-3 p-3 rounded-md bg-slate-50 border border-slate-200 text-xs"
                            >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
                    <div className="bg-white max-w-md w-full rounded-lg p-6 shadow-xl space-y-4">
                        ...
                        <p className="text-xs text-slate-500">
                            ...
                            ? Approval marks request as READY FOR STOCK
                            PROCESSING.
                        </p>
            ...
            {supplementModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
                    <div className="bg-white max-w-lg w-full rounded-lg p-6 shadow-xl space-y-4">
                        <div>
                            <h3 className="text-base font-bold text-slate-900">
                                Edit Approved Request (Admin Actions)
                            </h3>
                            <div className="p-3 bg-amber-50 border border-amber-200 rounded-md mt-2 text-xs text-amber-900">
                                <strong className="font-bold">
                                    ⚠️ Notice:
                                </strong>{" "}
                                Administrative edits are limited to Plant, G/L
                                Account, PWO No., Pur Org, Pur Group, and Cost
                                Center. All changes require a reason and will
                                record BEFORE vs AFTER values in the Audit
                                Trail.
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSupplement} className="space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    />
                                    {request.gl_account !==
                                        supplementForm.data.gl_account && (
                                        <div className="text-[10px] text-red-600 font-semibold mt-0.5">
                                            Diff:{" "}
                                            {request.gl_account || "empty"} →{" "}
                                            {supplementForm.data.gl_account}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    />
                                    {request.pwo_no !==
                                        supplementForm.data.pwo_no && (
                                        <div className="text-[10px] text-red-600 font-semibold mt-0.5">
                                            Diff: {request.pwo_no || "empty"} →{" "}
                                            {supplementForm.data.pwo_no}
                                        </div>
                                    )}
                                </div>
                                ...
                                <div>
                                    ...
                                    />
                                    {request.cost_center !==
                                        supplementForm.data.cost_center && (
                                        <div className="text-[10px] text-red-600 font-semibold mt-0.5">
                                            Diff:{" "}
                                            {request.cost_center || "empty"} →{" "}
                                            {supplementForm.data.cost_center}
                                        </div>
                                    )}
                                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
                    <div className="bg-white max-w-md w-full rounded-lg p-6 shadow-xl space-y-4">
                        ...
                        <p className="text-xs text-slate-500 leading-relaxed">
                            Cancelling an approved request marks status as{" "}
                            <strong className="text-red-700">
                                CANCELLED_AFTER_APPROVAL
                            </strong>
                            . If a STOCK_OUT transaction already occurred, an
                            automatic REVERSAL transaction will be rendered to
                            restore inventory SOH.
                        </p>
